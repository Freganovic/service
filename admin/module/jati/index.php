        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped" id="example1">
                ...
                <tbody>
                    ...
                    // Query untuk mengambil data dari tabel "jati"
                    $query = "SELECT * FROM jati";
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Jati</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form-tambah-jati">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="btn-simpan-jati">Simpan</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        // Tangani pengiriman data jati saat tombol "Simpan" di modal ditekan
        $('#btn-simpan-jati').click(function() {
            // Validasi input data jati di sini jika diperlukan
            var dataJati = {
                tanggal: $('#tanggal').val(),
                nama: $('#nama').val(),
            ...
            // Kirim data jati menggunakan AJAX
            $.ajax({
                type: 'POST',
                url: 'alat/proses_tambah_jati.php',
                data: dataJati,
                success: function(response) {
                    if (response === 'success') {
                        // Data berhasil ditambahkan, lakukan sesuatu jika diperlukan
                        alert('Data Jati berhasil disimpan');
                        ...
                        alert('Gagal menyimpan data Jati: ' + response);
                    }
                }
            });
        });
