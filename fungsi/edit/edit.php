...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['pemitra'])) {
        $nama= htmlentities($_POST['pemitra']);
        $id= htmlentities($_POST['id']);
        $data[] = $nama;
        $data[] = $id;
        $sql = 'UPDATE pemitra SET  nama_pemitra=? WHERE id_pemitra=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../index.php?page=pemitra&uid='.$id.'&success-edit=edit-data"</script>';
    }
